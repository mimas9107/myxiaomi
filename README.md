...
## 近期技術更新 (0.3.7)
- **DNS storm 排除**：確認根因為 DNS spoof 規則過寬，導致 vacuum 落入 Cloud retry loop 並產生高頻 DNS 查詢。
- **Fake Cloud 路徑收斂**：改採「最小必要攔截」，只處理控制相關 endpoint，不再攔截整個 Xiaomi 主網域。
- **部署重點明文化**：補齊 Router DNS 與 dnsmasq 的必備設定，讓 LAN-First 架構在無外網時仍可穩定控制。
